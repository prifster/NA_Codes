...
%Mapping of nodes of individual elements in global coordinates 
...
K(ndnumb,ndnumb) = K(ndnumb,ndnumb) + ke; % updaing global-stiffness matrix
end
...
for i=1:nb1
    ...
    n = cos(0);
    VerticalForce = VerticalForce + int((P(HBBN(i,1)) - P(HBBN(i,2)))*n,0,L);
end
VerticalForce = vpa(2*VerticalForce,4);
disp("Vertical Force on Body Boundary")
disp(VerticalForce);

% Added Mass Calculation
AdMB = vpa(real(VerticalForce)/(omega*omega),5);
disp("Added Mass");
disp(AdMB);
...
D = vpa(imag(VerticalForce)/(density*V*omega*omega),4);
