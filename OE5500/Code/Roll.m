...
% updaing global-stiffness matrix
...
for i=1:nb2
    syms theta
    L = abs(nodeheave(VBBN(i,1),2) - nodeheave(VBBN(i,2),2));
    n = cosd(theta);
    HForce = HForce + int(int((P(VBBN(i,1)) - P(VBBN(i,2)))*n,0,L),0,90);
end
HForce = vpa(2*HForce,4);
...
VForce = 0;
for i=1:nb1
    L = abs(nodeheave(HBBN(i,1),1) - nodeheave(HBBN(i,2),1));
    n = sind(theta);
    VForce = VForce + int(int((P(HBBN(i,1)) - P(HBBN(i,2)))*n,0,L),0,90);
end
VForce = vpa(2*VForce,4);
disp("Vertical Force on Body Boundary")
disp(VForce);

Force = VForce + HForce;
% Added Mass Calculation
AdMB = vpa(real(Force)/(omega*omega),5);
disp("Added Mass");
disp(AdMB);
...
D = vpa(abs(imag(Force))/((density^2)*V*omega*omega),4);
